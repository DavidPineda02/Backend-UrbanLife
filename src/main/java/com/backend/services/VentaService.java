...
public class VentaService {
    ...
    public static JsonObject create(String fechaVenta, String metodoPago,
        ...
        // Verificar que la fecha de venta sea exactamente la fecha de hoy (no se permiten fechas pasadas ni futuras)
        if (!fechaVenta.trim().equals(LocalDate.now().toString())) {
            // Indicar que la operación falló
            respuesta.addProperty("success", false);
            // Mensaje indicando que solo se permite la fecha actual
            respuesta.addProperty("message", "La fecha de venta debe ser la fecha de hoy");
            // Código HTTP 400 Bad Request
            respuesta.addProperty("status", 400);
            // Retornar respuesta de error
            return respuesta;
