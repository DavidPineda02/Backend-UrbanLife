...
// Servicio con la lógica de negocio de teléfonos de usuario
import com.backend.services.TelefonoUsuarioService;
...
/**
 * Controller que maneja los endpoints de teléfonos de usuarios.
 * Proporciona operaciones para listar, agregar y eliminar teléfonos del perfil.
 * Cada usuario solo puede gestionar sus propios teléfonos.
 */
public class TelefonoUsuarioController {

    /**
     * Handler para GET /api/telefonos-usuario.
     * Retorna todos los teléfonos del usuario autenticado.
     * @return HttpHandler que procesa la solicitud de listar teléfonos
     */
    public static HttpHandler listByUsuario() {
        return exchange -> {
            // Log de la petición recibida en consola
            System.out.println("Peticion: " + exchange.getRequestMethod() + " /api/telefonos-usuario");
            ...
            // Delegar al servicio la obtención de todos los teléfonos del usuario
            JsonObject respuesta = TelefonoUsuarioService.findByUsuarioId(usuarioId);
    ...
    /**
     * Handler para POST /api/telefonos-usuario.
     * Agrega un nuevo teléfono al perfil del usuario autenticado.
     * @return HttpHandler que procesa la solicitud de agregar teléfono
     */
    public static HttpHandler create() {
        return exchange -> {
            ...
            // Extraer el teléfono del JSON, usar "" si no viene en el body
            String telefono = datosJson.has("telefono") ? datosJson.get("telefono").getAsString() : "";

            // Delegar al servicio la validación y creación del teléfono
            JsonObject respuesta = TelefonoUsuarioService.create(telefono, usuarioId);
            // Extraer el código HTTP interno del servicio para usarlo en la respuesta
            int codigoHttp = respuesta.get("status").getAsInt();
            // Eliminar el campo "status" interno antes de enviar al cliente
            respuesta.remove("status");
    ...
    /**
     * Handler para DELETE /api/telefonos-usuario?id=X.
     * Elimina un teléfono del perfil del usuario autenticado.
     * Verifica que el teléfono pertenezca al usuario antes de eliminar.
     * @return HttpHandler que procesa la solicitud de eliminar teléfono
     */
    public static HttpHandler delete() {
        return exchange -> {
            ...
            int idTelefono = Integer.parseInt(parametrosUrl.split("=")[1]);
            ...
            // Delegar al servicio la verificación de propiedad y eliminación del teléfono
            JsonObject respuesta = TelefonoUsuarioService.delete(idTelefono, usuarioId);
            // Extraer el código HTTP interno del servicio para usarlo en la respuesta
            int codigoHttp = respuesta.get("status").getAsInt();
            // Eliminar el campo "status" interno antes de enviar al cliente
            respuesta.remove("status");
