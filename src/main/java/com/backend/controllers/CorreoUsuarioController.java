...
// Servicio con la lógica de negocio de correos de usuario
import com.backend.services.CorreoUsuarioService;
...
/**
 * Controller que maneja los endpoints de correos adicionales de usuarios.
 * Proporciona operaciones para listar, agregar y eliminar correos del perfil.
 * Cada usuario solo puede gestionar sus propios correos adicionales.
 */
public class CorreoUsuarioController {

    /**
     * Handler para GET /api/correos-usuario.
     * Retorna todos los correos adicionales del usuario autenticado.
     * @return HttpHandler que procesa la solicitud de listar correos
     */
    public static HttpHandler listByUsuario() {
        return exchange -> {
            // Log de la petición recibida en consola
            System.out.println("Peticion: " + exchange.getRequestMethod() + " /api/correos-usuario");
            ...
            // Delegar al servicio la obtención de todos los correos del usuario
            JsonObject respuesta = CorreoUsuarioService.findByUsuarioId(usuarioId);
    ...
    /**
     * Handler para POST /api/correos-usuario.
     * Agrega un nuevo correo electrónico al perfil del usuario autenticado.
     * @return HttpHandler que procesa la solicitud de agregar correo
     */
    public static HttpHandler create() {
        return exchange -> {
            ...
            // Extraer el correo del JSON, usar "" si no viene en el body
            String correo = datosJson.has("correo") ? datosJson.get("correo").getAsString() : "";

            // Delegar al servicio la validación y creación del correo
            JsonObject respuesta = CorreoUsuarioService.create(correo, usuarioId);
            // Extraer el código HTTP interno del servicio para usarlo en la respuesta
            int codigoHttp = respuesta.get("status").getAsInt();
            // Eliminar el campo "status" interno antes de enviar al cliente
            respuesta.remove("status");
    ...
    /**
     * Handler para DELETE /api/correos-usuario?id=X.
     * Elimina un correo adicional del perfil del usuario autenticado.
     * Verifica que el correo pertenezca al usuario antes de eliminar.
     * @return HttpHandler que procesa la solicitud de eliminar correo
     */
    public static HttpHandler delete() {
        return exchange -> {
            ...
            int idCorreo = Integer.parseInt(parametrosUrl.split("=")[1]);
            ...
            // Delegar al servicio la verificación de propiedad y eliminación del correo
            JsonObject respuesta = CorreoUsuarioService.delete(idCorreo, usuarioId);
            // Extraer el código HTTP interno del servicio para usarlo en la respuesta
            int codigoHttp = respuesta.get("status").getAsInt();
            // Eliminar el campo "status" interno antes de enviar al cliente
            respuesta.remove("status");
